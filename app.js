...
const port = 3030
const app = express();
const { addUser, addSimpleUser, addToken, checkUserData, recupToken, updateUser, updateToken, addBook, nomUserCheking, checkPDFExistance, sendProfilAsking, recupAllProfilsAsking, recupAllUploadAsking, activeParentControl, validDoc, validUpdate, totalCost, checkUserEmail, checkPDFExistanceByTheme } = require('./dbConnect')
const { sendEmail } = require('./emailConfig')
const { createToken, modifiedToken, extractTokenInfos, emptyTokenInfo } = require('./userToken')
...
app.use("/signIn", async (req, res) => {
    ...
    try {
        ...
        if (nomUserCheked) {
            console.log("Erreur : Nom Utilisateur Existant, veuillez en choisir un autre svp")
            errorMsg = "Erreur : Le nom d'Utilisateur " + nomUser + " existe, veuillez en choisir un autre svp";
            throw console.error();
        }
        // Sinon, 
        else {
            ...
            const decodedToken = JSON.parse(Buffer.from(tokenUpdated.split('.')[1], 'base64').toString('utf-8'));

            // Envoie de la reponse de vlaidation de l'inscritpion au frontend 
            res.status(200).send({ "message": 'Inscription en Argent Validée !', "userToken": decodedToken });
        }
    } catch (error) {
        // Gestion des erreurs
...
app.use("/silverSignIn", async (req, res) => {
    ...
    try {
        ...
        if (nomUserCheked) {
            console.log("Erreur : Nom Utilisateur Existant, veuillez en choisir un autre svp")
            errorMsg = "Erreur : Le nom d'Utilisateur " + userName + " existe, veuillez en choisir un autre svp";
            throw console.error();
        }
        // Sinon 
        else {
            // Insertion d'un Utilsiateur dans la base
            const { userId, user } = await addUser(userEmail, age, niveauEtude, classAnnée, password, npi, userName, sexe, tel);
...
app.use('/askUploadPDF', upload.single('pdf'), async (req, res) => {
    ...
    try {
        ...
        let annee = "", niveauEtude = "", classAnnee = "", chapterTab = [], tagTab = [], domainEtude = "", auteur = "", prix = "", ecole = "", ISBN = "", isFree = ''
        ...
        if (docCheked != null || docCheked != " ") {
            ...
            await addBook(destinationPath, fileName, domainEtude, annee, niveauEtude, classAnnee, chapterTab, tagTab, auteur, ISBN, prix, ecole, isFree)
            ...
            res.status(200).send({ message: 'Fichier PDF téléversé avec succès. En attente de la validation par un Administrateur' });
        ...
        // Envoyé une erreur au frontend
            ...
            console.log("Erreur : Le Document " + fileName + " existe, veuillez en choisir un autre svp.")
            errorMsg = "Erreur : Le Document " + fileName + " existe, veuillez en choisir un autre svp.";
            throw error;
        }
...
app.use("/profilAsking", async (req, res) => {
    ...
    try {
        ...
        if (user.profil != null) {
            ...
            const AskingInfoId = await sendProfilAsking(userName, user.profil, newProfil)
    ...
    try {
        ...
        // On procede au telechargement 
            ...
            // Si il n'y a pas d'erreur alors 
                ...
                if (isUpdate)
                    console.log('Token mis a jour avec succes.')
                res.status(200).send({ "message": "Doument téléchargéa avec succès.", "doc": doc, "token": decodedToken });
            }
...
app.use('/studyProfilAsking', async (req, res) => {
    ...
    try {
        ...
        // Si l'utilisateur ne dispose pas du oprofil adapté
            ...
            res.status(500).send({ message: 'Erreur Utilisateur sans autorisation.' });
...
app.use('/studyUploadAsking', async (req, res) => {
    ...
    try {
        ...
        if (user.profil == "Animateur") {
            ...
            // console.log(allAsking)
            res.status(500).send({ message: 'Felicitation', uploadAsking: allAsking });
        ...
        // Affichage d'erreur et envoie au frontend
            ...
            res.status(500).send({ message: 'Erreur Utilisateur sans autorisation. Vous n\'etes pas un admin' });
...
app.use('/seachingDoc', async (req, res) => {
    ...
    try {
        ...
        const files = await checkPDFExistanceByTheme(typeDoc, keyWord, optionSearch)
        ...
        // On envoie l'erreur au frontend 
            ...
            res.status(404).send({ message: 'Aucun document trouvé pour Domaine d\'étude', "Docs": files });
...
app.use('/buyDoc', async (req, res) => {
    ...
    try {
        ...
        if (isUpdate)
            console.log('Token mis a jour avec succes.')
        res.status(200).send({ "message": "Doument ajouté au panier avec succes.", decodedToken });
...
app.use('/payDoc', async (req, res) => {
    ...
    try {
        ...
        if (isUpdate)
            console.log('Token mis a jour avec succes.')
        res.status(200).send({ "message": "Extraction des Documents effectuée avec succes.", "infosPanier": basketInfos, "prixTotal": amount, "token": decodedToken });
...
app.use('/validPubDoc', async (req, res) => {
    ...
    try {
        ...
        if (user.profil == "Animateur") {
            ...
            res.status(500).send({ message: 'Publication du Document valider avec succes.' });
...
app.use('/activeParentControl', async (req, res) => {
    ...
    try {
        ...
        if ((parent.profil == "Parent" || parent.profil == "Animateur") && child.profil == "Client") {
            ...
            if (activated) {
                ...
                res.status(500).send({ message: 'Control parental activer avec succes.' });
            } else {
                console.log("Erreur lors de l'activation du control parental.")
                res.status(500).send({ message: 'Erreur lors de l\'activation du control parental.' });
            }
...
app.use('/getAllDocDownloadByMyChild', async (req, res) => {
    ...
    try {
        ...
        if ((parent.profil == "Parent" || parent.profil == "Animateur") && child.profil == "Client" && child.controlState == true) {
            ...
            if (activated != null) {
                ...
                res.status(500).send({ message: 'Informations recuperer avec succes.', infos: activated });
            } else {
                console.log("Erreur lors de la recuperation des infos.")
                res.status(500).send({ message: 'Erreur lors de la recuperation des infos.' });
            }

        } else {
            console.log("Erreur control Parental non activé ou Utilisateur sans autorisation.")
            res.status(500).send({ message: 'Erreur control Parental non activé ou Utilisateur sans autorisation.' });
...
app.use('/validProfilUpdate', async (req, res) => {
    ...
    try {
        ...
        if (admin.profil == "Animateur") {
            ...
            const token = await recupToken(user.tokenId)
            const newToken = await modifiedToken(token, validated.updatedUser, dataType)
            ...
            if (validated && isUpdate) {
                ...
                res.status(500).send({ message: 'Mise à jour de profil valider avec succes.', token: decodedToken });

            } else {
                console.log("Erreur : Mise à jour de profil non effectuée.")
                res.status(500).send({ message: 'Erreur : Mise à jour de profil non effectuée.' });
